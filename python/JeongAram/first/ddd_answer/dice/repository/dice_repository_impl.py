...
# 반드시 인터페이스 클래스에 있는 함수들을 만들어줘야 합니다.
class DiceRepositoryImpl(DiceRepository):
    # Singleton(싱글톤) 객체를 만들기 위한 기법 시작
    # 싱글톤이 되면 객체는 유일성을 가지게 됩니다. -> super()
    ...
    # 한번 만들어진 객체를 계속 유지하려고 한다 -> 싱글톤 특성
    # 없으면 새로 만들고 있으면 return 한다. 생성 후 return을 했다는 것은 객체가 만들어졌다는 의미

    # __new__를 보면 제일 이상한 부분이 보이는데 cls라는 것입니다.
    # 도대체 cls는 무엇일까요 ?
    # 보편적으로 cls는 클래스와 메서드가 서로 참조하기 위해 사용하는 내부 인자입니다.
    # self랑은 무슨 관계지 ? 라는 생각이 보편적으로 들 수 있습니다.
    # self는 초기화가 끝나야지만 사용할 수 있다.
    # 원래 객체화가 되었다면 self를 사용했겠지만
    # __new__() 를 진행하는 과정에서 아직 객체화가 완료되지 않았습니다.
    # 그렇기 때문에 self를 사용할 수 없는데
    # 이 시점에 클래스 자체에 접근할 수 있는 인자가 바로 cls입니다.

    def __new__(cls):
        if cls.__instance is None:
            cls.__instance = super().__new__(cls)   # super(): parent 클래스 상속 -> DiceRepository
    ...
    def checkDice(self, playerId):  # java 스타일 코드
