...
class ProductRepositoryImpl(ProductRepository):
    ...
    def __new__(cls):
        if cls.__instance is None:
            ...
            # 만약 싱글톤이 아닌데 이런 형식으로 구성했더라도 코드가 작동하긴 하겠지만
            # 본인의 의도와는 다르게 흘러갈 수 있으므로 언제나 명시적인 코드를 작성하도록 노력해야 함
            cls.__instance.__fixedProductList = []
            cls.__instance.__fixedProductList.append(
                Product("상품1", 500, "상품1은(는) 500원입니다.")
            )
            cls.__instance.__fixedProductList.append(
                Product("상품2", 1000, "상품2은(는) 1000원입니다.")
            )
