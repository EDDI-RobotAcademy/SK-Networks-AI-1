...
class DiceRepositoryImpl(DiceRepository):
    ...
    # cls가 뭘까? // 보편적으로 cls는 클래스와 메서드가 서로 참조하기 위해 사용하는 내부 인자입니다.
    # self랑은 무슨 관계지? 라는 생각이 보편적으로 들 수 있다.
    # 원래 객체화가 되었다면 self를 사용하겠지만 __new__()를 진행하는 과정에서 아직 객체화가 완료되지 않았습니다. 
    #그렇기 때문에 self를 사용할 수 없는데 이 시점에 클래스 자체에 접근 할 수 있는 인자가 바로 cls입니다.
    ...
    def rollDice(self, playerId, playerNickname):
        ...
        dice.setPlayerNickname(playerNickname) 
