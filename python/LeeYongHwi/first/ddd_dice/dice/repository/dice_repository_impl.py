...
class DiceRepositoryImpl(DiceRepository):
    # Singleton(싱글톤) 객체를 만들기 위한 기법 시작 (한 번 만든 객체를 계속 유지하려고 하기 때문에 싱글톤)
    # 싱글톤이 되면 객체는 유일성을 가지게 됩니다.
    # Entity에 있는 객체들은 여러 개가 될 수 있습니다.
    # 반면 이들을 서포트하는 Repository 객체나 Service 객체가 여러 개 존재할 필요가 없습니다.
    ...
    def __new__(cls):
        if cls.__instance is None:
            cls.__instance = super().__new__(cls)  # 부모 클래스를 객체화
    ...
    def getInstance(cls):
        if cls.__instance is None:
            cls.__instance = cls()  # 여기서 cls는 자기 자신이므로 __new__(cls)가 실행됨
