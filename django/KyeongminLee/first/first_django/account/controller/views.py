...
from account.serializers import AccountSerilaizer
...
class AccountView(viewsets.ViewSet):
    ...
    def checkEmailDuplication(self, request):
        ...
        try:
            ...
            return Response({'isDuplicate': isDuplicate, 'messgage': 'Email이 이미 존재' \
                             if isDuplicate else 'Email 사용 가능'}, status=status.HTTP_200_OK)
        except Exception as e:
            print('이메일 중복 체크 중 에러')
    ...
    def registerAccount(self, request):
        try:
            ...
            serializer = AccountSerilaizer(account)
