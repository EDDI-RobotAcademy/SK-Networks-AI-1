...
class PolynomialRegressionRepositoryImpl(PolynomialRegressionRepository):
    ...
    def regressionAnalysis(self, X, y):
        ...
        # 다항 피처 변환 후에는 [1, x1, x2, x1^2, x1 * x2, x^2, ...] 과 같은 형태를 가지게 된다.
        ...
        # 이런 복잡한 영역은 선형 회귀가 불가능 합니다. (왜냐면 비선형이기 때문입니다.)
        # 보편적으로 화확 분석, 재료 분석 같은 것을 할 때 많이 사용합니다.
        # EX) 배터리를 만들 때 보편 A랑 B랑 1:1로 섞으면 뭐가 된다 합니다.
        # 대용량으로 만들기 시작하면 1:1 비율이 틀어지게 됩니다.
        # (이런 것이 대표적인 비선형적인 문제인데 <- 논문 쓰기엔 좋지만 이걸로 문제 해결이 안됨)
        # 사실 이건 인간이 노가다로 비율 맞춰야 한다. (10년동안 배터리 비율 맞추는 노가다를 했다는 전설이 있다...)
        # 결론 비선형 문제는 살짝 비현실적 허상 같은 느낌이 있다.
        # -> 학교에서 편미분 방정식을 배우면 재밌는 사례까 있는데 경계 조건이 딱딱 들어맞으면 아주 나이스하게 풀 수 있다.
        # -> 그러나 현실 문제에서는 저런게 없다.
        ...
        # 결론: ax^2 + bx + cx + d를 예측했다 보면 된다.
        # -3 ~ 3 사이을 100개의 균일한 값으로 구성하고 100행 1열로 변환
