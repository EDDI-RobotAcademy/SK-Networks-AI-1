...
logisticRegressionRouter = APIRouter()      #얘는 곧 domain의 urls.py와 같다.


# @GetMapping
# @RequestMapping(get)
@logisticRegressionRouter.get("/logistic-regression")     #@와 바로 아래 있는 함수들은 세트이다.
def logistic_regression_test():
    print("logistic_regression_test()")

    np.random.seed(0)
    X = np.random.randn(100, 2) # 임의의 (x, y) 2차원 벡터(좌표)를 100개 생성
    y = (X[:, 0] + X[:, 1] > 0).astype(int)    # x 좌표와 y 좌표를 더해서 0 보다 크면 1, 아니면 0
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=0)    # 자동으로 100개 중 30% 를 테스트 셋으로 지정함
    ...
    x_values = np.linspace(X[:, 0].min(), X[:, 0].max(), 100)   #x의 최소값부터 최대값까지의 구간 중 100등분을 해서 선형의 공간을 나눔
    ...
    # ln -> w.x + b = 0
    # w1.x1 + w2.x2 + ... + b = 0
    y_values = -(coef[0][0] * x_values + intercept[0]) / coef[0][1]   #분류를 위한 결정경계를 구하는 식
