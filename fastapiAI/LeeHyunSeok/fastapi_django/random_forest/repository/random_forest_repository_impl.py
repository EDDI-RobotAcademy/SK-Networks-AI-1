...
class RandomForestRepositoryImpl(RandomForestRepository):
    ...
    def train(self, X_train, y_train):
        #트리를 100개 생성
        #램덤 포레스트의 경우 여러 개의 결정 트리를 구성하게 됩니다.
        #좀 더 정확하게 100개의 결정 트리를 만들어보겠다는 의미로 보면 되겠습니다.
        ...
        #데이터가 많은 상황에서 이상한 학습이 진행되는 과적합이 방지된다는 이점이 큼
        #결정 트리의 숫자가 많을수록 전반적인 성늘이 좋아지지만
        #일정 숫자 이상이 되면 성능 개선이 미미하며
        #응답 속도가 떨어지기 때문에 여러모로 트레이드 오프가 중요함

        #랜덤포레스트는 어떤 상황이든 과적합이 방지된다(왜냐? 평균을 내기 때문에)
    ...
    #불균형 데이터셋에서 소수 클래스의 데이터를 증각하여 데이터 균형을 맞춤 (불균형이라는 것이 포인트)
