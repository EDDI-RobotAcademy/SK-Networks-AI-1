...
class PrincipalComponentAnalysisRepositoryImpl(PrincipalComponentAnalysisRepository):
    ...
    def createPCASample(self):
        ...
        return numberOfPoints,numberOfFeature,numberOfComponents
    ...
    # numberOfFeature가 5에 해당하므로 5x5 형태의 정방 행렬이 생성됩니다.
    ...
    def configCovariance(self, numberOfFeatures):
        ...
        covariance = np.dot(covariance, covariance.transpose())     #covariance을 공분산 행렬을 만들기 위해 작업을 하는 것
        print(f"covariance: {covariance}")

        return covariance


    # 아래 코드에서 mean파트는 특징 모두의 평균을 0으로 만듭니다.(이 부분은 쉬움)
    # 다중 확률 분포 함수를 구성할 떄는 여러 확률 변수들에 대한 평균이 필요합니다.
    ...
    # PCA = 주성분 분석을 위한 Scikit learn 라이브러리의 제공 함수입니다.
