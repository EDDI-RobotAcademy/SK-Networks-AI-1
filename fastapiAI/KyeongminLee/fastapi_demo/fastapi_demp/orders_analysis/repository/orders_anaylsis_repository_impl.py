...
from orders_analysis.repository.orders_anaylsis_repository import OrdersAnalysisRepository

import tensorflow as tf

class OrdersAnalysisRepositoryImpl(OrdersAnalysisRepository):
    # viewCount와 qunantity 열을 추출하여 독립 변수 X와 종속변수 y로 사용
    # StandardScaler()를 통해 표준화(Normalization)
    async def prepareViewCountVsQuantity(self, dataFramse):
        X = dataFramse['viewCount'].values.reshape(-1, 1)
        y = dataFramse['quantity'].values
    ...
    # 현재 상황이 데이터 특정이 안되기 때문에 Dropout을 버리는 것은 다소 위험할 수 있음
    ...
    async def fitModel(self, model, X_train, y_train):
        model.fit(X_train, y_train,
                  ...
                  batch_size=32,
                  verbose=0) # verbose=1 : 'loss/metric/...'학습과정 보기

    # 입력데이터를 스케일링함
    # StandardScaler로 만들어진 것을 joblib을 이용해서 저장했었음
    # 이를 사용하여 0~1사이의 정규 분포 구성으로 설정함
    ...
    # flastten()을 사용하여 1D 배열로 변경하여 첫 번째 요소를 반환
