...
# Response가 완료될 때 까지 전체 자원을 혼자 다 독차지 하고 있다는 것이 문제가 되는 것임
...
# 모든 서버가 터지고 뻗는 대형 사고가 발생하기 때문

# *Blocking
#  CPU 자원을 계속 붙잡고 놓지 않는 상황

# *Non-Blocking
#  Blocking과는 조금 다른데 예로 한 순간에 1000개가 들어오지만
#  3시간 동안 아무런 입력이 없는 경우가 있을 수 있음
#  보편적으로 Polling 방식을 이용해서 Blocking을 하면
#  매 순간 '이거 있어?' '저거 했어?' 같은 것을 묻는 것임
#  Non-Blocking 방식은 '나 이거 했다, 여유 되면 가져가'
#  조금 정신 없다가 나중에 확인하고 '어, 가져갈게, 나는 요거 했어, 필요하면 가져가'
#  또 다른 녀석들이 와서 'ㅇㅋ 확인함, 난 요거 했고 저거 해서 가져가'
