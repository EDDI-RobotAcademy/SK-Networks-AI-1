...
def logistic_regression_test():
    ...
    # 회귀 분석이란?
    # 결론적으로 y = A.X + B 를 차즌ㄴ 수학적인 기법이라 볼 수 있음
    # 기본적으로 증명을 위해선 편미분을 하용해야하나 ... 여기는 수학 학원이 아니니 배제
    # 기본적인 접근은 x 값에 대해 최소 오차가 얼마인지, y 값에 대한 최소 오차가 얼마인지를 판별하기 위해
    # 특정 방향 도함수(편미분)를 사용해야하는 것임
    # 미분하니 사실 좀 어질어질 할 수 있는데 복잡하게 생각할 필요 없이 미분은 변화율이라 생각하면 됨
    # 그러니 변화가 얼마나 적게 일어나는가를 기준으로
    # 함수를 어떻게 만들 것인가 추적하는 기법으로 정리할 수 있음.

    # 테스트용 좌표를 가지고 결과 값을 추론
    y_pred = model.predict(X_test)

    # 예측치인 pred 값과 실제 테스트 셋인 test를
    # 비교하여 정확도가 어느 정도인지 판별
    accuracy = accuracy_score(y_test, y_pred)
    # y = ax + b라 가정했을 때 coef 는 a를 의미함 (기울기)
    coef = model.coef_
    # y = ax + b에서 intercept_ 는 b를 의미함 (y절편)
    intercept = model.intercept_

    # x 값을 일정 범위로 설정
    # 100개의 x 성분 중 최소 값과 최대 값을 뽑아서 일정 간격으로 분할
    ...
    return JSONResponse(
        content={
            ...
            "data_point": {"X": X.tolist(), "y": y.tolist()},
            "decision_boundary": {
