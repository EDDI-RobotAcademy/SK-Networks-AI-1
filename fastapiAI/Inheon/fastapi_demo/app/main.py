...
# 웹 브라우저 상에서 "/" 을 입력하면 Hello World 가 리턴
@app.get("/")
def read_root():
    return {"Hello": "World"}
...
# 실질적으로 router(controller) 역할을 하는 녀석들을 분리할 필요학 있음.
# 이것도 최소한이고 REST API 특성상
# service, repository, controller 가 동일하게 필요함.
# 그러나 우선 요번 케이스에서는 controller 만 구성하도록 함
# 추가적으로 Vue + Django 상황에서는 이야기 하지 않았지만
# DTO (request_form, request, response, response_form) 도 필요함
# DTO 라는 용어조차 사실 다소 근본없다 생각하므로
...
# 그럼 갑자기 궁금한 것이 생김
# request와 request_form 의 차이는 뭔가?
