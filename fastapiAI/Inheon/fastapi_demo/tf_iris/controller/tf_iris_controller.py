...
SCALER_PATH = 'tf_iris_scalar.pk1'
...
async def tfTrainModel():
    ...
    # Dense() 의 경우 Fully Connected 를 의미함
    ...
    # Softmax 를 사용하는 근본적인 이유는 입력 벡터를 확률 분포로 변환하기 위함임
    ...
    # 부가적으로 우리가 사용하는 아래 형태가 결국 Neural Network 라는 신경망에 해당함
    # Back Propagation라는 것이 가능하려면 결국 함수 자체가 미분 가능해야 하는데
    # exponential 함수 구성은 미분 가능이므로 신경망에서 사용할 수 있음
    ...
    # 최소 10차항까지는 계산을 해줘야 근사치를 얻을 수 있기 때문에 연산 자체는 무거음
    # 이런 부분에서 GPUI가 활약하면 성능을 상당히 많이 개선할 수 있음
    # 이 목적으로 논의되었던 것이 반도체 분야에선 NPU 혹은 DPU
    # 현재 NPU 시장이 가장 큰 업체는 APPle에 해당함
    ...
    # 구성된 모델의 성능을 평가할 지표가 필요한데 해당 지표로 accuracy(정확도)를 사용
    model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
    # 훈련 모델 학습
    # epochs는 몇 회 반복 학습을 시키느냐에 해당하는 수치
    # 500번 반복 학습을 시키니 iris라는 꽃 분야에 상당한 대가로 거듭나게 훈련시키는 것
...
def predict(tfIrisRequestForm: TfIrisRequestForm):
    if not os.path.exists(MODEL_PATH) or not os.path.exists(SCALER_PATH) or not os.path.exists(CLASSIFICATION_PATH):
        raise HTTPException(status_code=400, detail="train 부터 진행해주세요. 모델 및 스케일러 준비 안됨")
