...
class PolynomialRegressionServiceImpl(PolynomialRegressionService):
    ...
    async def generateSampleData(self):
        ...
        print(f"y: {y}")
        X = X[:, np.newaxis] # [1, 100] 1행 100열 이였던 것 -> [100, 1]
        print(f"after X: {X}")

        # 행렬의 연산 조건 (n X m) . (k x p)
        # 기본적으로 m과 k가 일치해야 연산할 수 있으므로 차원 이동시킨 것
        # 물론 pseudo 행렬은 저거 안 맞아도 할 수 있긴 하지만 이건 너무 넘어감
        return X, y
    async def createSamplePolynomialRegression(self):
