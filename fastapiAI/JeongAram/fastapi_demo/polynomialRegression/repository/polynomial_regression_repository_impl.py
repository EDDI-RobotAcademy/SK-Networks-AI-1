...
class PolynomialRegressionRepositoryImpl(PolynomialRegressionRepository):
    ...
    def regressionAnalysis(self, X, y):
        ...
        # 좀 더 엄밀하게는 차원 분석을 시켜서 degree 값도 판정하게 해야한 우선 그냥 진행
        # 다항 형식의 Feature 추출
        ...
        #    degree=3 이면 3차 함수, 4면 4차 함수라 보면 되겠다.
        #    여기서 이제 조금 더 나가면 x, y, z 가 물리는 경우가 발생할 수 있는데
        #    이런 복잡한 영역은 선형 회귀가 불가합니다. (왜냐하면 비선형적이기 때문)
        #    보편적으로 화학 분석, 재료 분석 같은 것을 할 때 많이 사용합니다.
        #    예로 배터리 만들 때 보면 A랑 B랑 1:1로 섞으면 뭐가 된다 합니다.
        #    근데 막상 대용량으로 만들기 시작하면 1:1 비율이 틀어지게 된다.
        #    (이런 것이 대표적인 비선형적인 문제인데 <- 논문 쓰기엔 좋지만 이걸로 문제 해결이 안됨)
        #    사실 이건 이간이 노가다로 비율 맞춰야한다.
        #    결론이 비선형 문제는 살짝 비현실적 허상 같은 느낌이 있다.
        #    -> 학교에서 편미분 방정식을 배우면 재밌는 사례가 있는데
        #       경계 조건이 딱딱 들어맞으면 아주 나이스하게 풀 수 있다.
        #    -> 그러나 현실 문제에서는 저런게 없다.
        ...
        model.fit(X_Polynomial, y)  # 특징 추출
        ...
        # 각 다항 피처에 대한 y값 예측
        y_pred = model.predict(X_new_poly)  # 모델 학습

        # test data는 학습을 시키면 안됨
        X_new = np.linspace(-3, 3, 100).reshape(-1, 1)  # 테스트 데이터
        X_new_poly = polynomialFeature.transform(X_new)  # X_new로 다향식 뽑기 -> test data 아님
        y_pred = model.predict(X_new_poly)

        # flatten() 이 붙은 케이스들은 전부 1차원 형태로 만들고 리스트화하여 변환
        # flatten()은 행과 열을 서로 바꿔준다.? -> (n,m)이었던 행렬을 n행 1열로 만드는 것
