...
# 대표적으로 Spring과 Spring Reactive 가 차이가 있음
# Spring과 결이 같이 녀석들이 Flask, Django에 해당함
# Spring Reactive와 결이 같은 녀석은 FastAPI라고 보면 됨

# 둘의 가장 큰 차이는 Request 이후 Response 까지 Blocking이 발생하는지 안하는지 여부에 해당함
# 이전에 Multi-Tasking이나 context Switchting 이야기를 하면서
# Task들이 서로 자원(CPU, Memory, 기타 등등)을 획득하기 위해 전쟁을 치루는 것을 볼 수 있었음
# 가장 큰 문제점이라면 자우너은 한정적인데 어떤 녀석이 Request 이후
# Response 가 완료 될 때까지 전체 자원을 혼자 다 독차지 하고 있다는 문제가 되는 것임
...
# *Non-Blocking
# Blocking과는 조금 다른데 예로 어떤 한순간에는 1000개가 들어오지만
...
# 또 다른 녀석들이 와서 " ㅇㅋ 확인함, 난 요거 했고 저거 했어 가져가"
...
# 다른 서버들의요청에 대한 응답 처리를 하지 못하고 TimeOut 뜨면서 모든 것이 터지는 상황이 발생함
# axios에 Timeout을 2500(2.5초) 굉장히 길게 줬는데 Blocking 하면 2.5초는 우습게 씹어먹고 다 터짐

# 고로 Event Driven 방식을 사용해서 Non-Blocking 방식으로
# "야 나 지금 이거 했어, 필요하면 가져가"히고 다른 것을 대기하지 않고 바로 다름 작업에 착수하게 됨
# 사실 지금 우리가 사용하고 있는 애자일 보드나, 이슈 시스템, 슬랙 묶어 놓은 것도 이 방식이긴 함
...
async def polynomialRegression(polynomialRegressionService: PolynomialRegressionServiceImpl =
    ...
    X, y, X_new, y_pred = await polynomialRegressionService.createSamplePolynomialRegression()
