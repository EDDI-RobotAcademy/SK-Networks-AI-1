...
class LanguageModelRepositoryImpl(LanguageModelRepository):
    ...
    # 변환된 문자 인덱스를 TensorFlow에서 사용할 수 있는 형태로 만듬
    ...
    def createDataSet(self, text, textAsIndex):
        ...
        # from_tensor_slices()는 배열을 텐서 플로우가 사용할 수 있는 형태의 배열로 만든다
        # 예로 [32, 1, 54, 23, 17, 3] 이러면 동일하게 32, 1, 54, 23, 17, 3을 순차적으로 반환함
        sequenceList = characterDataSet.batch(self.SEQUENCE_LENGTH + 1, drop_remainder=True)
        # 그리고 이 부분 때문에 사실 정확도가 떨어지게 됨
        # 상용의 경우엔 이 부분을 SEQUENCE 값을 조정하여 여러 번 진행하게 만듬
    ...
    def trainModel(self, sequenceList, characterList):
        ...
        # 같은 데이터 셋을 가지고 학습하면 학습 정확도가 떨어질 수 있기 때문에 랜덤한 배치를 이용하여
        ...
        # vocabularySize는 문자 집합의 크기(집합은 중복을 허용하지 않음)
        ...
        # 들더라도 밥을 입 속이 아닌 어디 이상한데다가 넣고
        # 이것도 몇 번씩 하다 보면 결국 입속으로 숟가락을 가져가게 됩니다.
        ...
        print(f"model: {model}")
        # checkpoint 는 모델을 학습시키다가 터지는 이슈를 방지하기 위해서 사용함
