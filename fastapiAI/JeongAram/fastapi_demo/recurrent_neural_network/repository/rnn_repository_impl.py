...
class RecurrentNeuralNetworkRepositoryImpl(RecurrentNeuralNetworkRepository):
    ...
    def createRnnModel(self, vocabSize, embaddingDimension, rnnUnits, batchSize):
        ...
        model = Sequential([
            # Embedding은 text를 vector로 사상(map) 하는 작업입니다.
            # 그렇기에 사실 엠베딩 보다는 mapToVector 혹은 textToVector 또는 TextRepresentation이 더 좋은 것 같긴합니다.

            # Embedding Layer에서는 vocabSize를 지정하여 입력 차원을 지정합니다.
            # text 데이터에서 사용하는 고유 단어의 수가 vocabSize 만큼이라는 뜻입니다.

            # embaddingDimension의 경우 출력 차원에 해당합니다.
            ...
            Embedding(vocabSize, embaddingDimension, batch_input_shape=[batchSize, None]),
            ...
            # (전형적인 피드백 형태)
            # <- 다른 것에 대비하여 데이터 품질이 굉장히 중요한 녀석 중 하나 (데이터셋이 이상하면 전체에 영향을 주기 때문)
        ...
        # softmax 를 쓰지 않는 이유는 text를 만들어낼 것이기 때문이다.
    ...
    def generateText(self, loadedRnnModel, inputText):
        ...
        # 생성된 텍스트를 저장
        generatedText = []
        # 무작위성을 제어하기 위한 엔트로피용 변수 (보편적으로 값이 낮을수록 더 예측 가능하며 높을수록 무작위성이 높아짐)
        ...
        for index in range(numGenerate):
            ...
            # rnn 구조 상 마지막 값이 학습이 가장 잘 된 값이므로, 마지막 batch의 정보만 활용한다.
            predictedId = tf.random.categorical(entropicalPredictions, num_samples=1)[-1, 0].numpy()

            tfInputEval = tf.expand_dims([predictedId], 0)  # (predicted = 1) -> (1, 1) 로 확장
            generatedText.append(chr(predictedId))  # 숫자를 다시 문자화

        return inputText + ''.join(generatedText) # join: list를 str로 바꿔서 이어줌
