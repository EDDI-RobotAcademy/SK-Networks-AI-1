...
def train_test_evaluaion(selectedMetrics=None):
    ...
    print(f"irisFlower key: {irisFlower.keys()}")  # 딕셔너리 key값
    # sepal: 꽃받침, petal:꽃잎
    print(f"irisFlower feature: {irisFlower.feature_names}") # 특징
    print(f"irisFlower target: {irisFlower.target_names}")  # 어디에 속하는지 분류하는 작업
    ...
    for key in classReport.keys():
     ...
     # 웹 페이지 상에서 정보를 주고 받을 땐 그냥 무조건 json 변환한다 생각하자
    return JSONResponse({
        "accuracy": accuracy,
