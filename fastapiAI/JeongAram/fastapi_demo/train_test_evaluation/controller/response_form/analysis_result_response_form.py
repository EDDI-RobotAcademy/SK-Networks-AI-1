...
# 경우에 따랏 account와 product 정보의 일부를 추출해서 줘야할 수 있음
# 그럼 '어떻게 줄 것이냐 ?' 라는 문제에 봉착하게 됨

# 퉁치는 경우를 생각해보자
# 전부 다 하나에 때려박은 다음에 보냈다.
# 근데 새로운 비즈니스 및 마켓팅에 의해 봰줘야 하는 정보의 내용이 추가되었다.
# 혹은 일부는 뺴내야하고 버려야한다.
...
# 여기서도 Responsedhk ResponseForm이 나뉘어지는데
# 보편적으로 Response는 특정 Domain 정보를 추출하기 위한 목적으로 사용함
# 고로 ResponseForm은 도대체 무엇이냐? 라는 의문이 여전히 남음

# 실제로 우리가 데이터를 본은 장소는 웹 페이지 이다.
# 그렇기 떄문에 웹 페이지에서 보여주고자 하는 Format(형식)이 존재한다.
# 바로 이 형식에 맞춰주는 것이 ResponseForm이다.

# 반대로 RequestForm의 경우도 있는데
# 웹 페이지에서 보내는 정보가 특정 Domain에 국한 된 것이 아니라
# 여러 Domain 정보가 한 번에 몰려 들어올 수 있기 때문이다.
# 대표적으로 주문(order)라는 것이 역시 그러하다.
# 주문 시 주문과 관렪나 정보로 주문 번호, 주문 고객,
# 주문 상품, 결제 금액, 배송 관련 등등의 정보들이 전부 결함 될 수 밖에 없다.
