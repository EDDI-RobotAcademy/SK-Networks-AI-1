...
class ReviewAnalysisRepositoryImpl(ReviewAnalysisRepository):
    ...
    def preprocess(self, xData, yData, englishStop):
        ...
        # 알파벳을 제외한 나머지는 삭제하겠다
        # '\s'는 띄어쓰기 -> 이걸 안해주면 단어가 띄어쓰기 안되고 다 붙어서 출력됨
        xOnlyAlphabetData = xRemovedHtmlTagData.replace({'[^A-Za-z\s]': ''}, regex=True)

        # 사용 데이터(xExcludeStopData)는 실제 pandas로 인해 DataFrame 상태에 해당함
        # apply lambda를 통해서 각 프레임의 review들에 대해 모두 적용
        # reveiw.split()으로 텍스트를 공백 기준으로 나누어 단어 단위로 분석함
