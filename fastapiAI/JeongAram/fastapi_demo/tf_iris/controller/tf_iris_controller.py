...
MODEL_PATH = 'tf_iris_model.h5' # 최고의 지식인 상태를 저장하기 위한 용도
SCALER_PATH = 'tf_iris_scaler.pk1' # 가장 잘 나왔을 때 scaler 값
CLASSIFICATION_PATH = 'tf_iris_classification_label.pk1' # label 정보 -> 최종 클래스 이름 알려고 저장 (setosa인지)
...
async def tfTrainModel():
    ...
    scaler = StandardScaler() # scaler로 불균형 normalize 시켜줌
    ...
    # 이것이 '세종 대왕님꼐서 화가 많아 맥북 던짐' 으로 연결됨
    ...
    # 치소 10차항까지는 계산을 해줘야 근사칠르 얻을 수 있기 때문에 연산 자체는 무거움
    ...
    # 현재 DPU / NPU 시장이 가장 큰 없체는 Apple에 해당함
    # 그리고 짐 켈러가 요즘 RISC V를 가지고 뭔가를 하려고해서 Nvidia 거품이 꺼질 전망임
    model = tf.keras.Sequential([
        tf.keras.layers.Dense(64, activation='relu', input_shape=(4,)), # input_shape는 직접 지정(특징 4개)
    ...
    # optimizer : 손실 함수의 최적화 - adam이 방향도 고려해줘서 좋다?많이 쓰임
    model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy']) # loss를 어떤 방식으로 학습할지
    # 훈련 모델 학습
    # epochs는 몇 회 반복 학습을 시키느냐에 해당하는 수치이다.(하이퍼 파라미터 = 우리가 정할 수 있는 파라미터)
    # 500번 반복 학습을 시키니 iris 라는 꽃 분야에 상당한 대가로 거듭나게 훈견시키는 것
...
@tfIrisRouter.post('/tf-predict') # 여기가 찐 테스트
def predict(tfIrisRequestFrom: TfIrisRequestForm):
    ...
    data = np.array([
        [
            tfIrisRequestFrom.sepal_length,
            tfIrisRequestFrom.sepal_width,
            tfIrisRequestFrom.petal_length,
            tfIrisRequestFrom.petal_width,
        ]
    ])
    ...
    whichOneIsMax = np.argmax(prediction) # argmax: 현재 list 안에 있는 최대값의 index
    ...
    return {
        "prediction": prediction[0][whichOneIsMax].tolist(), # 몇 번째가 가장 클지 모르기 때문에 값을 지정해주지 않고 변수로 지정
        "predicted_class": predictedClass
    }
    # classification하고 나면 한번 train 하고 난 후 껐다 켜도 학습된 것이 남아있기 때문에 다시 학습시켜줄 필요가 없다.
    # -> train & forgot