...
#그러나 order만 생각해도 벌써 accoun와 product오 ㅏ연동하고 있음
...
#결이 딱 정해져 있는 것이 아니라 이것저것 해보면서 데이터도 이것저것 드러가게 됨
# 그러다 보니 사실 변경이 매우 빈번하게 발생함.
# 근데 실질적으로 모든 기능들이 다 엮어 있다보니 저런 변경 상황에 유연성을 잃어버리게 됨
#고로 다 분리하자는 관점에서 Response, ResponseForm, Request, RequestForm이 만들어짐

#여기서도 Response와 ResponseForm이 나뉘어지는데
#보편적으로 Response는 특정 Domain 정보를 추출하기 위한 목적으로 사용함
#고로 'ResponseForm은 도대체 무엇이냐?' 라는 의문이 여전히 남음

#실제로 우리가 데이터를 보는 장소는 웹페이지 입니다.
#그렇기 때문에 웹페이지에서 보여주고자 하는 Format(형식)이 존재합니다.
