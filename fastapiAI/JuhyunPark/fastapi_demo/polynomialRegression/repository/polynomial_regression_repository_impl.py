...
class PolynomialRegressionRepositoryImpl(PolynomialRegressionRepository):
    ...
    def regressionAnalysis(self, X, y):
        ...
        # 다항 피쳐 변환 후에는 [1, x1, x2, x1^2, x1 * x2, x^2, ...] 과 같은 형태를 가짐

        # Q. 선형 회귀는 y = x 형태고, 다항 회귀는 100개 축이 있으면 100차원인지?
        # A. 우선 기저(축)이 100 개 있으면 100차원이 맞습니다.
        # 그런데 우리는 degree=2를 줬기 때문에 2차 함수에 대한 분석이라 보면 되겠습니다.
        # degree=3 이면 3차함수, 4면 4차 함수라 보면 되겠습니다.
        ...
        # 결론은 비선형 문제는 살짝 비현실적 허상 같은 느낌이 있습니다.
        #
        # -> 학교에서 편미분 방정식을 배우면 재밌는 사례가 있는데
        #   경계 조건이 딲딱 들어맞으면 아주 나이스하게 풀 수 있음
