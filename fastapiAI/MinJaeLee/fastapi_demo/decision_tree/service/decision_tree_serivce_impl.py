...
class DecisionTreeServiceImpl(DecisionTreeService):
    ...
    def decisionTreeTrain(self):
        print(f"service -> decisionTreeTrain()")
        ...
        scaledTrainDataFrame, sclaedTestDataFrame = self.decisionTreeRepository.applyStandardScaler(trainDataFrame,
                                                                                                    testDataFrame,
                                                                                                    wineInfo.feature_names)

        trainDataFrameAfterSlice, testDataFrameAfterSlice = self.decisionTreeRepository.sliceTensor(
            scaledTrainDataFrame, sclaedTestDataFrame)
