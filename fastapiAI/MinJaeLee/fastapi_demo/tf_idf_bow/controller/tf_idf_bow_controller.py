...
# cosine_similarity()를 사용하여 messageVector와 conntMatrix사잉의 유사도를 계산합니다
# cos의 특성상 -1~1의 범주를 가지며 어차피 음수는 필요가 없으므로 0~1의 값을 가지고 계산합니다
# 특정 데이터가 (3, 5)이고 (4,4)라고 한다면, cos각도 상의 차이가 발생합니다
# 이를 토대로 유사도를 파악하는 시스템이라 보면 되겠습니다.
# 즉 기준을 대상으로 90도가 되면 완벽하게 일치한다 판정할 것입니다.
# cos(0)rhk cos(90)은 -1과 1이기 때문입니다.
