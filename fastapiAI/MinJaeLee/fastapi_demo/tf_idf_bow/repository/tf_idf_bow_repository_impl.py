...
documentTitleListForTest = [
    ...
    "나중에 또 살펴보디 프랑스가 뉴스에서 한국에 일장기를 그려넣었다.",
...
class TfIdfBowRepositoryImpl(TfIdfBowRepository):
    ...
    # 여기서 TOP_RANK_LIST를 3개로 지정하여 유사도가 높은 걳중 3개를 뽑습니다.
    # 또한 제약 사항으로 Throsholdd(임계치)를 줘서 유사도 10% 이상인 것만 추출하도록 만들었습니다. ㄹ
    def findSimilar(self, message, countVectorizer, countMatrix):
        print(f"repository -> findSimilar()")
        ...
        # 그 중 TOP_RANK에 속하는 3가지만 뽑습니다.
        similarIndiceList = cosineSimilarityList.argsort()[-self.TOP_RANK_LIST:][::-1]
        # 그 중에서도 THRESHOLD 값이 최소 입계치(10%) 이상인 녀석들만 다시 추출합니다.
        similarDocumentList = [(documentTitleListForTest[index], cosineSimilarityList[index]) for index in
                               similarIndiceList if cosineSimilarityList[index] >= self.SIMILARITY_THRESHOLD]

        return similarDocumentList

    # 실질적으로 문장을 벡터화 하는 녀석입니다
    def documentVectorization(self):
        print(f"repository -> documentVectorization()")
        countVectorizer = CountVectorizer()
        # 상위의 배열에 기록되어 있는 문저 제목에 해당하는 문장을 가지고 벡터화를 진행합니다.
