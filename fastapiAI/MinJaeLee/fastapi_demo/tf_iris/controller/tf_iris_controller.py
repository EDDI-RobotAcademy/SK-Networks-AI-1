...
async def tfTrainModel():
    ...
    # 훈련 모델 정의
    # Sequential을 통해 모델의각 Layer를 순차적으로 구성함을 의미함
    # 즉 적혀있는 순서대로 다음 Layer를 타고 내려가게 됨
    # Dense()의 경우 Fully Connected를 의미함
    # 첫 번째 Layer는 64개의 뉴런을 가지고있고 입력 크기는 4개에 해당함
    # 여기서 입력 크기라는 것은 파라미터가 4개 존재한다는 것을 의미함
    # 활성 함수로 'relu'를 사용하는데 relu는 y=x와 같은 성형적인 특성을 가지고 있으면 사용하는 편
    ...
    # 최소 10차 항까지는 계산을 해줘야 근사치를 얻을 수 있기 때문에 연산 자체는 무거움
    # 이런 부분에서 GPU가 확약하면 성능을 상당히 많이 개선할 수 있음
    # 이 목적으로 논의되었던 것이 반도체 분야에선 NPU(Neural-network Processing Unit)  혹은 DPU(Deep-learning Processing Unit)라고 불러왔음
    # 현재 DPU/NPU 시장이 가장 큰 업체는 Apple에 해당함
    # 그리고 짐 켈러가 요즘 RISC V를 가지고 뭔가를 하려고 해서 Nvidia 거품이 꺼질 전망임
    ...
    # 몇가지 팁: 학습을 했을 때 데이터가 들쭉날쭉하는 경향을 보일 수 있음
    #           아무리 학습을 많이 해도 데이터 자체가 파동을 만든다는 뜻임(수렴하지 않고)
    #           만약 이런 경우가 나타난다면 relu를 사용하지 않고 tanh를 사용하는 것을 권장함
    #           기본적으로 하이퍼볼릭 탄젠트(tanh)의 경우 쌍곡선 함수로 cosh와 sinh 로 구성됨
    #           그리고 하이퍼볼릭 함수들을 자체적으로 exponential을 가지고 있기 때문에
    #           위와 같은 곡선형 데이터, 파동형 데이터를 적합하는데 상당히 훌륭함
    #           단 위에서 거론했듯이 exponential이 많이 사용되면 어쩔 수 없이 수치해석을 해야하기 때문에
    #           컴퓨터 내부적으로 테일러급수 전개가 장제되고 이로 인해 학습 시간은 길어짐
    #           단, 추론 성능은 상당히 좋아지게 됨.
    #           어차피 우린 학습과 추론을 분리할 것이므로, 필요하면 쓰는것이 정답

    # 훈련모델 컴파일
    # optimizer는 수동으로 커스텀하여 배치할 수도 있으나
    # 이러한 부분을 직접 조정하는것을 하이퍼 파라미터 튜닝이라고 함
    # 우선은 기본적으로 쉽게 활용할 수 있는 모델로는 아래의 Adam 같은 것이 있음
    # 경사 하강법 기반을 사용하므로 학습 속도가 빠르고, 보편적인 성능을 가짐
    # sparese_categorical_crossentropy의 경우 라벨이 정수로 인코딩 된 경우 다중 분류 문제에 적합함
...
def predict(tfIrisRequestForm: TfIrisRequestForm = Body(...)):
    ...
    data = np.array([[tfIrisRequestForm.sepal_length, tfIrisRequestForm.sepal_width, tfIrisRequestForm.petal_length,
    ...
    return {
        "prediction":prediction[0][np.argmax(prediction)].tolist(),
