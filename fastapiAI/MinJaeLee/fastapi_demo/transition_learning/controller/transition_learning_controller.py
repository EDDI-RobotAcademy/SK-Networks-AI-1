...
from transition_learning.controller.request_form.gpt2_pretrained_prediction_request_form import \
    GPT2PretrainedPredictRequestForm
...
async def predictWithTransitionLearning(transitionLearningPredictRequestForm: TransitionLearningPredictRequestForm,
                                        transitionLearningService: TransitionLearningServiceImpl = Depends(
                                            injectTransitionLearningService)):
    print(
        f"controller -> predictWithTransitionLearning(): " f"transitionLearningPredictReqestForm: {transitionLearningPredictRequestForm}")
...
async def transitionLearningWithSentimentAnalysis(sentimentAnalysisRequestForm: SentimentAnalysisRequestForm,
                                                  transitionLearningService: TransitionLearningServiceImpl = Depends(
                                                      injectTransitionLearningService)):
    print(
        f"controller -> transitionLearningWithSentimentAnalysis():" f"sentimentAnalysisRequestForm: {sentimentAnalysisRequestForm}")
...
@transitionLearningRouter.post("/gtp2-pretrained-prediction")
async def gpt2PretrainedPrediction(gpt2PretrainedPredictionRequestForm: GPT2PretrainedPredictRequestForm,
                                   transitionLearningService: TransitionLearningServiceImpl = Depends(
                                       injectTransitionLearningService)):
    print(
        f"controller -> transitionLearningWithSentimentAnalysis():" f"gpt2PretrainedPredictionRequestForm: {gpt2PretrainedPredictionRequestForm}")
