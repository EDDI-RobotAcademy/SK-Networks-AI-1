...
class TransitionLearningServiceImpl(TransitionLearningService):
    ...
    def predictText(self, transitionLearningPredictRequestForm):
        ...
        # 전통적인 방식은 이전에 저희가 작업했듯이 단어(word), 문자(character)단위로 전부 학습을 시키고
        ...
        modelName, tokenizer, model = self.__transitionLearningRepository.prepareBertBasedUncasedLearningSet()
        model.eval()

        # 입력한 두 문장을 위의 tokenizer를 통해 토큰화하여 모델에 입력합니다
        # BERT는 문장을 토큰으로 변환하고, 각 토큰을 임베딩 벡터로 변환합니다.
        tokenizedUserInputText = tokenizer(transitionLearningPredictRequestForm.firstSentence,
                                           transitionLearningPredictRequestForm.secondSentence, return_tensors="pt",
                                           truncation=True)

        # 모델은 두 문장을 인코딩 한 이후 두 문장의 관계를 나타내는 벡터를 구성합니다.
        # 이벡터를 사용하여 두 문장이 의미적으로 동일한지 여부를 판단합니다.
    ...
    def transitionLearningWithSentimentAnalysis(self, sentimentAnalysisRequestForm):
        print(f"service -> transitionLearningWithSentimentAnalysis()")

        modelName, tokenizer, model = self.__transitionLearningRepository.prepareBertBaseMultilingualUncasedSentimentLearningSet()
        model.eval()

        tokenizedUserInputText = tokenizer(sentimentAnalysisRequestForm.sentence, return_tensors="pt", truncation=True)
    ...
    def predictTextWithGPT2(self, gpt2PretrainedPredictionRequestForm):
        print(f"service -> predictTextWithGPT2()")

        modelName, tokenizer, model = self.__transitionLearningRepository.prepareGPT2PretrainedLearningSet()
        model.eval()

        # tokenizedUserInputText = tokenizer(gpt2PretrainedPredictionRequestForm.text, return_tensors="pt")

        encodedInputList = tokenizer.encode(gpt2PretrainedPredictionRequestForm.text, return_tensors="pt")
        outputList = model.generate(encodedInputList, max_length=1000, num_return_sequences=1)
