...
# 즉 위 형식은 q 변수에 test를 넣겠단 소리
# 또한 vue에서의 가변 인자와는 조금 다르게
# 여기서 가변 인자는 아래와 같이 {item_id} 를 중괄호로 감쌈
...
# 왜냐하면 모든 로직은 main에 전부 다 때려박았기 때문
...
# DTO 라는 용어가 근본이 없다 생각하는 이유는
# 나중에 현업 수준으로 구성이 커지게 되면 Request도 엄청나게 많아지고
# response도 엄청난 분량으로 늘어나게됨
# 그럼 잠깐 머뭇하면서 이게 요청인가 응답인가 고민하면서 집중이 꺠짐
# 아마 우리 프로젝트를 하면서도 엄청나게 비대해지는 것을 볼 수 있을 것이라 생각됨
...
# 초반에 얘기하지 않은 이유는 '일단 만들자' <<< 가 중요해서
# 만들어 놓고 이런 부분이 '좀 짜치는데?' 하면서
# 점진적으로 개선시키는 것이 'Agile' 방식임
# 빠른 습득 및 생산성의 비결임
