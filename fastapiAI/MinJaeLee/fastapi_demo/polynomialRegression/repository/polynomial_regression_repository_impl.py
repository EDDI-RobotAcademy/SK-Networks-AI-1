...
class PolynomialRegressionRepositoryImpl(PolynomialRegressionRepository):
    def regressionAnalysis(self, X, y):
        ...
        #Q. 선형 회귀는 y = X 현태고, 다항회귀는 100개 축이 있으면 100차원인지?
        # A. 우선 기저(축)가 100개 있으면 100차원이 맞습니다.
        ...
        #   보편적으로 화학분석, 재료분석 같은것을 할때 많이 사용합니다
        #   예로 배터리 만들 때 보면 A랑 B랑 1:1로 섞으면 뭐가 된다 합니다.
        #   근데 막상 대용량으로 만들기 시작하면 1:1 비율이 틀어지게 됩니다.
        #   이런것이 대표적인 비선형적 문제인ㄷ
        #   사실 이건 인간이 노가다로 비율 맞춰야 합니다;;
        #   [ 그렇게 그.. 교수님의... 노...]     <- 10년간 대학원에서 졸업 못했다는 전설이...
        #   결론이 비선형적 문제는 살짝 비현실적 허상같은 느낌이 있습니다.
        #   -> 학교에서 편미분 방적식을 배우면 재밌는 사례가 있는데
        #       경계 조건이 딱딱 들어맞으면 아주 나이스하게 풀 수 있습니다.
        #   -> 그러나 현실 분제에서는 저런게 없어요.
        ...
        # fit 을 통해 위와 같은 데이터를 학습시킴
        model.fit(X_Polynomial, y)

        # 결론: ax^2 + bx + cx + d 를 예측했다 보면 됨
        #-3~3 사이를 100개의 균일한 값으로 구성하고 100행 1열로 변환
        X_new = np.linspace(-3, 3, 100).reshape(-1,1)
        # 새로 뽑은 X_new를 다항 피처로 변환함
