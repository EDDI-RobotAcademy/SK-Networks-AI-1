...
# 데이터를 저차원의공간으로 보낼 수 있는 매우 훌륭한 기법

# 이게 무슨 소리냐?
# 고차원의 데이터란 것은 결국 데이터를 판정할 때 사용한느 파라미터가 여러 개 있다는 뜻입니다.
# 쉽게 얘기해서 ax^n + bx^(n-1) + cx^(n-2) + ... + zzzx^(n-1004) + ...
# 위와 같은 형태로 표현 되는 케이스를 좀 더 단순화 시켜서 유사한 혹은 동일한 모양을 만들어 내는 것이죠
# 위 케이스는 결국 n차원이지만 pca를 통해서 이 n차원을 n-x차원으로 떨굴 수 있다는 것이 핵심임
# 설명을 위해 2차원으로 했지만 3차원, 4차원 등등 관여하는 종속 변수가 더 많을 수 있습니다

# 실제초 PCA 분석은 분산(Var)를 최대화 하는 축 (Principal Component)를 찾는 것이 핵심입니다
# Principal Component Analysis라고 이름 붙은 이유가
# 결국 분산을 최대화하는 주성분을 찾아 분석하는 기법이기 때문입니다.
# PCA는 결국 분산값을 최대화 하는 새로운 좌표계를 생성하게 됩니다.

# 이렇게 여러 종속 변수들이 존재하는 영역에서 반드시 다뤄지는 것이 있는데
# 바로 공분산이라는 Co-Variance라는 녀석입니다
# 실제로 PCA는 공분산 행렬을 사용하여 데이터의 분산과 상관 관계를 분석합니ㅏㄷ
# 공분산 행렬의 고유값(eigenvalue)과 고유벡터(eigenvector)를 계산하여 데이터 주성분을 찾습니다
# 학교에서 Linear Algebra(선형대수) 학습할 때 고유값 고유벡터는 들어봤을 겁니다
# 피보나치 수열의 일반화를 위해서도 고유값과 고유벡터를 사용하니까요

# 여기서 고유값이 크고 작은 것은 상당히 중요한 척도가 됩니다
# 고유값이 크면 클술록 해당 벡터 방향으로의 분산 값이 거대하다는 것입니다
# 위에도 적었듯 PCA는 분산값이 커야 합니다
# 고유벡터는 결국 이 고유값들을 모아놓은 벡터 형태가 되는데
# 데이터의 새로운 축을 나타내며 이 축을 따라 데이터가 가장 크게 분산됩니다.
# 아주 신기한 현상인데, 입으로만 얘기하면 사실 무슨얘기인지 모를 수 있습니다
...
# 말 그대로 Z 변환이라고 하는데 실제로 우리가 Tensoflow 및 Scikit Learn을 사용하면서 많이 봤던 StandardScaler() 가 바로 이 역할을 수행합니다
# 평균이 0, 분산이 1이 되도록 표준화 합니다.
# 그래서 옛날 수학의 정책에 보면 뒤쪽에 통계학쪽 도표가 그려져 있는것을 볼 수 있는데
# 바로 그것을 만들어주는 작업이 이 Z Transform(Z변환)에 해당합니다

# 실제 통계 책에서는 Z score를 아래와 같이 나타냅니다.
# z = (x - 뮤) / 시그마
# 뮤는 평균값을 의미하며 x는 각 데이터의 샘플임
# 그리고 시그마는 표준편차(Standard Deviation)에 해당합니다
...
async def pcaAnalysis(principalComponentAnalysisService: PrincipalComponentAnalysisServiceImpl = Depends(
    injectPrincipalComponentAnalysisService)):
