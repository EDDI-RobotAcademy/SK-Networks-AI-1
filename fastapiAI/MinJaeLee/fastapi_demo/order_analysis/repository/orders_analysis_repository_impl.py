...
class OrdersAnalysisRepositoryImpl(OrdersAnalysisRepository):
    ...
    # 현재 상황이 데이터 특정이 안되기 때문에 Dropout을 버리는것은 사실 다소 위험 할 수 있음
    async def createModel(self):
        model = tf.keras.models.Sequential([
            tf.keras.layers.Dense(128, activation='relu', input_shape=(1,)), tf.keras.layers.Dropout(0.3),
    ...
    # flatten()을 사용하여 1D 배열로 변경하여 첫번째 요소를 반환
