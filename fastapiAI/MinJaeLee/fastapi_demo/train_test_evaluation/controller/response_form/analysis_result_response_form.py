...
# 그러나 order만 생각해봐도 벌써 account와 product와 연동하고 있음
# 이런 경우 order 정보만 주는게 아니라
# 경우에 따라서 account와 product정보의 일부를 추출해서 쥐야 할 수 있음
...
# 혹은 일부는 빼내야하고 버려야한다.
# 재밌게도 마케팅이란 것이 이것도 해보고 저것도 해보고 사람들 반응도 보고 하다 보면
...
# 고로 다 분리하자라는 관점에서 Repsonse, ResponseForm, Request, RequsetForm이 만들어짐

# 여기서도 Response와 ResponseForm이 나뉘어지는데
# 보편적으로 Response는 특정 Domain 정보를 추출하기 위한 목적으로 사용함
# 고로 Response는 특정 Domain 정보를 추출하기 위한 목적으로 사용함
# 고로 ResponseForm은 도대체 무엇이냐? 라는 의문이 여전히 남음

# 실제로 우리가 데이터를 보는 장소는 웹 페이지.
# 그렇기 때문에 웹 페이지에서 보여주고자 하는 형식이 존재함
# 바로 이 형식에 맞춰주는것이 ResponseForm이다.

# 반대로 RequestForm의 경우도 있는데
# 웹 페이지에서 보내는 정보가 특정 Domain에 국한 된 것이 아니라
# 여러 Domain 정보가 한번에 몰려 들어올 수 있기 때문이다.
# 대표적으로 주문(order)라는 것이 그러합니다
# 주문시 주문과 관련한 정보로 주문 번호, 주문 고객, 주문 상품, 결제 금액, 배송 관련 등등의 정보들이 전부 결합 될 수밖에 없습니다.
# 이런 상황에서 각 Domain 별로 Request와 Response를 관리하려면
# 전체를 묶고 필요에 따라 특정 Domain의 Response, Request 전환이 가능하도록 구성하는 것이
# 코드의 가독성과 변경에 대한 유연성이 더욱 강화된다.
