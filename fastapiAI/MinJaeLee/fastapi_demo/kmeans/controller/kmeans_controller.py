...
# kmeans는 사용은 단순한데 비해 활용을 굉장히 다방면으로 할 수 있어서 좋은 녀석입니다.
# 우선 범용적으로 영화 장르를 분석 할 때
# 멜로, 액션, 공포 같은것이 있다고 해봅시다
# 10대들이 무엇에 가장 관심을 가지는가라고 단순하게 판정할 것이 아닌
# 각각의 구성원들의 취향이 있기 때문에
# 멜로 영화를 좋아하는 집단이 있을 것이고
# 액션 영화를 좋아하는 집단이 있고
# 공포 영화를 좋아하는 집단이 있으며
# 다 좋아하는 사람도 있습니다.
# 이런 경우 특정 사람들을 그룹핑 해주는 것이 바로 kmeans cluster 알고리즘 입니다.
# 취향 분석에 사용할 수 있다는 의미

# 그리고 별개로 군사적 목적으로 사용할 수도 있음
# 대표적인 예가 LRASM미사일임
# 보편적으로 미사일로 정밀 타격을 할때 군사용 위성을 사용하여 유도함
# (우리가 사용하는 GPS와는 정밀도가 급이 다름)
# 그런데 요즘 전자전이 발달하다보니, 통신 신호를 교라시킬 수 있음(재밍)
# 그래서 GPS신호를 교란하는 교란기들을 보편적인 항모전단은 운용을 함
# 그런데도 싸워야 하니까 이것을 우회할 방법으로 인공지능을 채택하였음
# GPS 재밍 지역에서는 미사일에 탑재된 전자파로 상대 전단의 레이더 신호를 감지함
# 그리고 사거리 밖으로 자동 라우팅 플랜(경로 계획)을 세움
# 그렇게 레이더에 감지되지 않는 거리 밖으로 이동함
# 이후 전자파에 잡히는 데이터들을 그룹핑함(이때, kmeans cluster를 사용)
# 그래고 타격해야 하는 목표물이 무엇인지 id를 부여해서 간파하기 시작함
# kmeans cluster에 의해 타겟이 확보되면 급강하를 진행함
# 저공비행을 하면 레이더에 잘 안걸리게 됨
# 그리고 달려가서 때려박음

# 이 당시 록히드 마틴에서 LRASM 미사일을 발표하고 곰돌이 푸가 화냈음
# 사실 요런 것을 보면 인공지능이 가장 많이 활용되는 분야가 군사 분야라는 것도 알 수 있음
# 생각보다 요즘 K 방산의 위력이 또 어마어마함
...
async def kmeans_cluster_analysis():
    ...
    # 위의 Response Model을 지정하면 아래와 같이 구성할 수 있음
    # 그러나 별로 권장하고 싶은 방식은 아님
    # 시스템이 커지고 Domain이 복잡해질수록 '뭐지?' 싶은 것들이 증대하게 됨
    # 그러나 세상에서 다양한 사람들을 만날수 있으니 알아둘 필요는 있음.
