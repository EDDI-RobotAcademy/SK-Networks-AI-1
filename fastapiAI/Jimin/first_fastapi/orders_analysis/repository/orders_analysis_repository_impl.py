...
# Layer는 Input, Hidden Layer, Output Layer로 구성됩니다.
...
# 1. 먼저 학습할 데이터를 확보합니다.
# 2. 전처리를 진행합니다.(쓸데없는 데이터, 이상치들 제거) (이상값=튀는 값들)
...
class OrdersAnalysisRepositoryImpl(OrdersAnalysisRepository):
    ...
    async def createModel(self):
        model = tf.keras.models.Sequential([ # 규칙성도 없고 데이터도 적기 때문에 relu를 쓸 필요가 없다.
            tf.keras.layers.Dense(128, activation='relu', input_shape=(1,)),# 조회수를 하나만, 입력 layer
            tf.keras.layers.Dropout(0.3),
            tf.keras.layers.Dense(64, activation='relu'), # 가운데 부분 hidden layer
            ...
            tf.keras.layers.Dense(1) # 출력 layer
        ]) # 과적합 방지를 위해 20%는 버리는 것을 선택
        # softmax를 사용하면 확률로 나온다.
        # 현재 우리는 몇개다. 라는 결과값이 필요하기 때문에 softmax(활성함수)를 사용하지 않았다.
    ...
    async def fitModel(self, model, X_train, y_train):
        model.fit(X_train, y_train, epochs=2000, validation_split=0.2, batch_size=32, verbose=0)
