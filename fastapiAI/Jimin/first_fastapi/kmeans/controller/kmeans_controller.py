...
# 멜로 영화, 액션 영화, 공포 영화 같은 것이 있다고 해봅시다.
# 10대들이 무엇에 가장 관심을 가지는가라고 단순하게 판정할 것이 아닌
# 각각의 구성원들의 취향이 있기 때문에
# 멜로를 좋아하는 집단, 액션을 좋아하는 집단, 공포를 좋아하는 집단, 다 좋아하는 집단이 있습니다.
...
# 그래서 GPS 신호를 교란하는 교란기들을 보편적인 항모전단을 운용 함
# 그런데도 싸워야 하니까 이것을 우회할 방법으로 인공지능을 채택했음
# GPS 재밍 지역에서는 미사일에 탑재된 전자파로 상대 전단의 레이더 신호를 감지함
# 그리고 사거리 밖으로 자동 라우팅 플랜(어떤 경로로 이동하겠다)을 세움
# 그렇게 레이더에 감지되지 않은 거리 밖으로 이동함
# 이후 전자파에 잡히는 데이터들을 그룹핑함(kmeans cluster를 사용함)
# 그리고 타격해야 하는 목표물일 무엇인지 id를 부여해서 간파하기 시작함
# kmeans cluster에 의해 타켓이 확보되면 급강하를 진행함
...
async def kmeans_cluster_analysis():
    ...
    # 데이터가 분포된 공간이 2차원이라면 sqrt(=루트)(x^2 +y^2) <- 피타고라스
    # 3차원이라면 sqrt(x^2 +y^2 + z^2) <- 피타고라스
    ...
    # 그러나 세상에서 다양한 사람들을 만날 수 있으니 알아둘 필요는 없음
