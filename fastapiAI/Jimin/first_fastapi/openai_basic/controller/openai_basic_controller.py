...
async def textSimilarityAnalysisWithOpenAI(openAIPaperSimilarityAnalysisRequestForm: OpenAIPaperSimilarityAnalysisRequestForm,
                                           openAIBasicService: OpenAIBasicServiceImpl =
                                           Depends(injectOpenAIBasicService)):

    print(f"controller -> textSimilarityAnalysisWithOpenAI(): openAITalkRequestForm: {openAIPaperSimilarityAnalysisRequestForm}")

    analyzedSimilarityText = await openAIBasicService.textSimilarityAnalysis(openAIPaperSimilarityAnalysisRequestForm.paperTitleList, openAIPaperSimilarityAnalysisRequestForm.userRequestPaperTitle)

    return JSONResponse(content={"result": jsonable_encoder(analyzedSimilarityText)}, status_code=status.HTTP_200_OK)