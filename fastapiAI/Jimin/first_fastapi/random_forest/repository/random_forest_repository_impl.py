...
class RandomForestRepositoryImpl(RandomForestRepository):
    ...
    def train(self, X_train, y_train):
        # 트리를 100개 생성
        # random forest의 경우 여러 개의 결정 트리를 구성하게 됩니다.
        # 100개의 결정 트리를 만들어 보겠다는 의미로 보면 되겠습니다.
        # 각각의 개별 트리는 데이터를 분할하여 예측을 수행할 수 있습니다.
        # 고로 총 100개의 예측들이 있다는 소리
        # 모든 트리는 (100개) 독립적으로 예측을 진행

        # 최종적으로 100개의 트리에서 나온 결과를 종합하여 결론을 내림
        # 각각의 트리들에서 확보한 평균값을 가지고 예측을 만듬

        # 데이터가 많은 상황에서 이상한 학습이 진행되는 overfitting(과적합)이 방지된다는 이점이 큼
        # 결정 트리의 숫자가 많을수록 전반적인 성능이 좋아지지만
        # 일정 숫자 이상이 되면 성능 개선이 미미하여 응답속도가 떨어지기 때문에 여러모로 트레이드 오프가 중요함
    ...
    # 이런 소수 집합에 성능을 높이고자 할 때 사용하면 효율적
