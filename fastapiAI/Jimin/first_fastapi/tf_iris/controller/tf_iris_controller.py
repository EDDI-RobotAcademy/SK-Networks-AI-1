...
from tf_iris.controller.reqeust_form.tf_request_form import TfIrisRequestForm
import numpy as np

tfIrisRouter = APIRouter()

# 가장 잘 나왔을 때의 모델과 스케일을 저장시켜놓고 test할때 이 값들을 가져다 씀
MODEL_PATH = 'tf_iris_model.h5'
SCALER_PATH = 'tf_iris_scaler.pk1'
# 클래스 이름(setosa인지, 다른거 인지)을 반환하기 위해
...
async def tfTrainModel():
    ...
    # 활성함수로 'relu'를 사용하는데 relu는 y = x 와 같은 선형적인 특성을 가지고 있으면 사용하는 편
    ...
    # 수학적으로 확률 분포 함수를 전 구간에 걸쳐 적분하면 1이 되기 때문(100%)
    ...
    # 부가적으로 우리가 사용하는 아래 형태가 결국 Neural Network라는 신경망에 해당함
    ...
    # 테일러 급수 전개를 해서 각각의 항들을 계산하고 팩토리얼 계산을 상수화아여 계산한다 하더라도
    # 최소 10차항까지는 계산을 해줘야 근사치를 얻을 수 있기 때문에 연산 자체는 무거움
    # 이런 부분에서 GPU가 활양하면 성능을 상당히 많이 개선할 수 있음
    ...
    #           단 위에서 거론했듯이 exponential이 많이 사용되면 어쩔 수 없이 수지해석을 해야하기 때문에
    #           컴퓨터 내부적으로 테일러 급숙 전개가 강제되고 이로 인해 학습 시간은 길어짐
    #           단, 추론 성능은 상당히 좋아지게 됨
...
def predict(tfIrisRequestForm: TfIrisRequestForm = Body(...)):
    ...
    # argmax: 현재 list안에 있는 최대 값의 index
