...
class LanguageModelRepositoryImpl(LanguageModelRepository):
    ...
    # 변환된 문자 인덱스를 TensorFlow에서 사용할 수 있는 형태로 만듭니다.(from_tensor_slices)
    # 마지막 문자는 제거하는 형식으로 구성함
    # 결론적으로 딥러닝 모델에 입력 할 수 있는 형태를 만들기 위한 작업
    ...
    def createDataSet(self, text, textAsIndex):
        exampleForEpoch = len(text) // self.SEQUENCE_LENGTH
        # exampleForEpoch는 결론적으로 주어진 데이터를 가지고 생성할 수 있는 문장 일부를 나타냄
        # 고로 100자씩 끊어치기 함
        characterDataSet = tf.data.Dataset.from_tensor_slices(textAsIndex)
        # from_tensor_slices()는 배열을 텐서가 플로우가 사용할 수 있는 형태의 배열로 만듬
        ...
        return exampleForEpoch, characterDataSet, sequenceList
    ...
    def trainModel(self, sequenceList, characterList):
        ...
        # vocabularySize는 문자 집합
        # Embedding을 사용하여 각 문자를 고정 크기의 벡터로 구성
        # 결국 vocabularySize가 임베딩 입력 차원에 해당함
        # batch_input_shape은 입력 데이터의 형태를 의미합니다 (앞서 문자를 추출 했을 때 64였으므로)
        # None은 문자들의 구성 자체가 가변적인 것을 의미합니다.
        # 한마디로 단어나 문장을 구성 할 때 문자의 개수가 일정하지 않음을 의미하는 부분입니다.

        # LSTM은 순환 신경망의 주요 구성으로 부분 문장을 처리합니다.
        # RNN_UNITS는 결국 LSTM Layer의 개수를 의미합니다.
        # 그러므로 지금 Hidden Layer가 1024개 있다는 뜻입니다.
        # 그리고 RNN의 특징은 피드백(FeedBack)입니다.
        # 실패 속에 경험치를 쌓으면서 목적한 바로 향하게 구성하는 작업이 피드백인데
        # 피드백 루프를 구성하기 위해 LSTM엔 return_sequence라는 옵션이 있습니다.
        # return_sequences를 True로 설정하면 부분 문장을 리턴하게 됩니다.
        # 고로 지속적으로 이것이 제대로 된 것인지 아닌지 피드백 하면서 갈 수 있게 됩니다.

        # stateful 옵션의 경우엔 모델이 배치 간의 상태를 유지하도록 만듭니다.
        # recurrent_initalizer에 glorot_uniform의 경우 순환 계산을 하도록 만듭니다.
