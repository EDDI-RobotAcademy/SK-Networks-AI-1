...
from principal_component_analysis.repository.pca_repository import \
    PrincipalComponentAnalysisRepository
import numpy as np
...
class PrincipalComponentAnalysisRepositoryImpl(PrincipalComponentAnalysisRepository):
    ...
    def createPCASample(self):
        numberOfPoints = 333 # 데이터 수
        numberOfFeatures = 5 # 5차원으로 늘렸다가
        numberOfComponents = 2 # 2차원으로 줄이겠다
    ...
    # 아래 방식과 같이 데이터 행렬과 데이터 행렬의 전치를 곱하면 됩니다. (증명은 생략)
    ...
    # numberOfFeatures가 5에 해당하므로 5 x 5 형태의 정방 행렬이 됩니다.
    ...
    def configCovariance(self, numberOfFeatures):
        ...
        # np는 행렬계산이 가능하다.
        # 2차원 리스트를 np.array를 씌우면 행렬이 된다.
        # np는 gpu를 쓸 수 있고 속도적인 유리함이 있다.
        # np는 행렬계산에 특화되어 있어서 고차원에서도 유리하다.
    ...
    def fitTransform(self, pca, createdDataFrame):
        # test 나 train 할때는 Transform
        # 훈련을 할 때만 fitTransform
