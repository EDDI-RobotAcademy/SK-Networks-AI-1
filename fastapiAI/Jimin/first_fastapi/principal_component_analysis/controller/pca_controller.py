...
from principal_component_analysis.service.pca_service_impl import \
...
# 위 케이스는 결국 n차원이겠지만 PCA를 통해서 이 n 차원을 n - x 차원으로 떨굴 수 있다는 것이 핵심입니다.
...
# 통계학 학습 할 때 Z Transform 이라는 것을 학습합니다.
...
# 그래서 옛날 수학의 정책에 보면 뒤쪽에 통계학쪽 도표가 그려있는 것을 볼 수 있는데
# 바로 그것을 만들어주는 작업이 이 Z Transform(Z 변환)에 해당합니다.

# 실제 통계책에서는 z score를 아래와 같이 나타냅니다.
# z = (x - 뮤) / 시그마
...
# 우선 대략 이러한 흐름을 잡고 PCA 분석을 살펴보면 이게 뭐하는 짓인지 대략은 파악 할 수 있음
# 결론적으로 종속 변수가 너무 많아서 정신 나가겠다 할 때 사용하면 좋다.


async def injectPrincipalComponenetAnalysisService() -> PrincipalComponentAnalysisServiceImpl:
...
async def pcaAnalysis(principalComponentAnalysisService: PrincipalComponentAnalysisServiceImpl =
                               Depends(injectPrincipalComponenetAnalysisService)):
