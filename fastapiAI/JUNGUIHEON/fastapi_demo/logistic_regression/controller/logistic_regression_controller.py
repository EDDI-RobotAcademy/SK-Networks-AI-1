...
def logistic_regression_test():
    ...
    # 기본적인 접근은 x 값에 대해 최소 오차가 얼마인지,
    # y 값에 대한 최소 오차가 얼마인지를 판별하기 위해
    # 특정 방향 도함수(편미분)을 사용해야하는 것임
    # 미분하니 사실 좀 어질어질 할 수 있는데 복잡하게 생각할 필요 없이
    # 미분은 변화율이라 생각하면 됨
    ...
    # 예측치인 pred 값과 실제 테스트 셋인 text를
    # 비교하여 정확도가 어느정도인지 판별
    accuracy = accuracy_score(y_test, y_pred)
    # y = ax + b 라 가정 했을 때 coef_ 는 a를 의미함
    coef = model.coef_
    # y - ax + b 에서 intercept_ 는 b를 의미함
    intercept = model.intercept_

    # x 값을 일정 범위로 설정!
    # 100개의 x 성분 중 최솟값과 최댓값을 뽑아서 일정 간격으로 분할
