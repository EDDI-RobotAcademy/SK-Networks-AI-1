...
from random_forest.repository.random_forest_reposiotry import RandomForestRepository
...
class RandomForestRepositoryImpl(RandomForestRepository):
    ...
    def evaluate(self, y_test, y_pred):
        ...
        confusionMatirx = confusion_matrix(y_test, y_pred)

        return accuracy, classificationReport, confusionMatirx
    ...
    def train(self, X_train, y_train):
        ...
        # 모든 트리는 (100개) 독립적으로 예측을 진행합니다.
        # 최종적으로 100개의 트링데서 나온 결과를 종합하여 결론을 내리게 됩니다.
        # 각각의 트리들에서 확보한 평균값을 가지고 예측을 만듦
        # 데이터가 많은 상황에서 이상한 학습이 진행되는 overfitting이 방지된다는 이점이 큼
        # 결정 트리의 숫자가 많을 수록 전반적인 성능이 좋아지지만
