...
async def tfTrainModel():
    ...
    # 훈련 모델 정의
    # Sequential을 통해 모델의 각 Layer를 순차적으로 구성
    # 즉 적혀 있는 순서대로 다음 Layer를 타고 내려가게 됨
    # Dense()의 경우 Fully Connected를 의미함
    # 첫 번째 Layer는 64개의 뉴런을 가지고 있고 입력 크기는 4개에 해당
    # 여기서 입력 크기라는 것은 파라미터가 4개 존재한다는 것을 의미
    # 활성 함수로 'relu'를 사용하는데 relu는 y = x와 같은 선형적인 특성을 가지고 있으면 사용하는 편
    # 그렇게 뉴런의 숫자를 줄여나가서 마지막에 활성 함수로 softmax를 사용함
    # softmax를 사용하는 근본적인 이유는 입력 벡터를 확률 분포로 변환하기 위함
    # 출력 벡터의 각 요소는 자체적으로 구성한 분류에 속하는 확률 자체를 나타냄
    # 또한 각각의 확률들의 총합은 1이 됨
    # 수학적으로 확률 분포 함수를 전 구간에 걸쳐 적분하면 1이 되기 때문
    # 결국 Softmax를 사용하면 A일 확률 33%, B일 확률 33%, C일 확률 34% 형태로 값이 나옴
    # 물론 위와 같이 나온다면 사용할 수 없음
    # A일 확률 98%, B일 확률 1%, C일 확률 1%와 같은 형태라면 상당히 준수한 것
    # 하지만 약점이 하나 있는데
    # 말도 안되는 없는 데이터를 넣는 경우에 대해 판정이 불가능
    # 부가적으로 우리가 사용하는 아래 형태가 결국 Nerual Network라는 신경망에 해당
    # Back Porpagation(역전파)이라는 것이 가능하려면 결국 함수 자체가 '미분 가능'해야 하는데
    # exponential 함수 구성은 '미분 가능' 이므로 신경망에서 사용할 수 있음
    # 근데 시그마 + exponential 연산 때문에 softmax 연산은 생각보다 무거운 편
    # 일단 exponential은 오일러 공식을 생각해 봐도 cos(x) + i sin(x) 형태를 띄기 때문에
    # 테일러 급수 전개를 해서 각각의 항들을 계산하고 팩토리얼 계산을 상수화하여 계산한다 하더라도
    # 최소 10차항까지는 계산을 해줘야 근사치를 얻을 수 있기 때문에 연산 자체는 무거움
    # 이런 부분에서 GPU가 활약하면 성능을 상당히 많이 개선할 수 있음
    # 이 목적으로 논의되었던 것이 반도체 분야에선 NPU(Neural-network Processing Unit)
    # 혹은 DPU(Deep-learning Processing Unit)라고 불러 왔음
    # 현재 DPU / NPU 시장이 가장 큰 업체는 Apple
    # 그리고 짐 켈러가 요즘 RISC V를 가지고 뭔가를 하려고 해서 Nvidia 거품이 꺼질 전망
    ...
    #            아무리 학습을 많이 해도 데이터 자체가 파동을 만든다는 뜻(수렴하지 않고)
    #            만약 이런 경우가 나타난다면 relu를 사용하지 말고 tanh를 사용하는 것을 권장
    #            기본적으로 하이퍼볼릭 탄젠트(tanh)의 경우 쌍곡선 함수로 cosh와 sinh로 구성됨
    #            그리고 하이퍼볼릭 함수들은 자체적으로 exponential을 가지고 있기 때문에
    #            위와 같은 곡선형 데이터, 파동형 데이터를 적합하는데 상당히 훌륭함
    #            단, 위에서 거론했 듯이 exponential이 많이 사용되면
    ...
    # sparse_categorical_crossentropy의 경우 라벨이 정수로 인코딩 된 경우 다중 분류 문제에 적합
    # 구성한 모델의 성능을 평가할 지표가 필요한데 해당 지표로 accuracy(정확도)를 사용
    model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
    # 훈련 모델 학습
    # epochs는 몇 회 반복 학습을 시키느냐에 해당하는 수치
    model.fit(X_train, y_train, epochs=100, validation_data=(X_test, y_test))

    # 훈련된 모델 저장(학습 완료 상태)
    model.save(MODEL_PATH)
    # 모델 및 스케일러를 불러오기 위해 사용
...
def predict(tfIrisRequestForm: TfIrisRequestForm):
    ...
    predictionMaxIndex = np.argmax(prediction)
    predictedClass = classificationLabel[predictionMaxIndex]
    print(f"predictedClass: {predictedClass}")

    return {
        "prediction": prediction[0][predictionMaxIndex].tolist(),
