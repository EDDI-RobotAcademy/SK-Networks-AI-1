...
class PrincipalComponentAnalysisRepositoryImpl(PrincipalComponentAnalysisRepository):
    ...
    def createPCASample(self):
        numberOfPonits = 333
        numberOfFeatures = 5
        numberofComponentes = 2

        return numberOfPonits, numberOfFeatures, numberofComponentes

    # 공분산은 실제 상관관계를 분석할 수 있는 아주 좋은 척도
    # 우리가 모델을 평가할 때 혼동행렬을 분석했 듯이
    # 각 종속 변수 간의 상호 연관성을 파악할 때는 이 공분산 행렬을 보면 됨
    # 공분산 공식은 Covariance(X, Y) = 1 / (n-1) sum[ (X샘플 - X평균) + (Y샘플 - Y평균) ]
    # 위와 같은 구성을 가지고 있음
    # 만약 위의 X샘플 - X평균이 이미 정규화 되었다면
    # 아래 방식과 같이 데이터 행렬과 데이터 행렬의 전치를 곱하면 됨
    # 결과적으로 공분산 행렬은 m x m 형태의 정방 행렬이 됨
    # 아래 코드에서 mean 파트는 특징 모두의 평균을 0으로 만듦
    # 평균을 이용해서 만들려고 했으나 그냥 랜덤으로 구성
    # 그러나 아래쪽에서 갑자기 공분산 파트가 다소 해석이 어려울 수 있음
    # rand() 함수 자체가 0과 1 사이의 균등 분포로 무작위 값을 생성
    # numberOfFeatures가 5에 해당하므로 5 x 5 형태의 정방 행렬이 생성됨
    # 위의 정규화에 이미 따르고 있으므로 공분산 행렬 계산 시 행렬 x 전치행렬 형태로 구성이 가능함
    ...
    # 주성분 분석을 위한 Scikit Learn 라이브러리의 제공 함수
    # 예로 원래 데어터가 10개의 변수를 가지고 있었다면
    # 지정한 n_components의 개수로 주성분 분석을 위한 준비를 함
