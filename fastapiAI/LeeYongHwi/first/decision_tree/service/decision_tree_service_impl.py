...
class DecisionTreeServiceImpl(DecisionTreeService):
    ...
    def decisionTreeTrain(self):
        ...
        scaledTrainDataFrame, scaledTestDataFrame = (
            self.decisionTreeRepository.applyStandardScaler(trainDataFrame, testDataFrame, wineInfo.feature_names))
        ...
        readyForLearnTrainData, readyForLearnTestData = (
            self.decisionTreeRepository.applyBatchSize(trainDataFrameAfterSlice, testDataFrameAfterSlice, 32))

        self.decisionTreeRepository.learn(readyForLearnTrainData)
