...
documentTitleListForTest = [
    ...
    "프랑스가 올림픽에서 한국을 북한이라고 소개하여 한국 네티즌들이 프랑스 국기에 나치 상징을 새겼다.",
...
class TfIdfBowRepositoryImpl(TfIdfBowRepository):
    ...
    def documentVectorization(self):
        ...
        # 이 계산 비용을 낮추려면 어떻게 해야될까?
        # 문장에 대한 데이터 -> 즉, 벡터를 미리 저장해놓고 사용하자
        # 미리 저장해놓으면 매번 계산할 필요가 없으므로 계산 비용을 크게 낮출 수 있음
        # 벡터를 저장해놓고 필요할 때 불러와서 코사인 유사도 계산만 하면 끝.
        # VectorDB의 탄생 배경. MongoDB, PostDB 등
    ...
    def findSimilar(self, message, countVectorizer, countMatrix):
        ...
        # 그 중에서도 THRESHOLD 값이 최소 임계치 (10%) 이상인 녀석들만 다시 추려냅니다.
