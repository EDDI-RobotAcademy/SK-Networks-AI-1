...
# 가장 큰 문제점이라면 자원은 한정적인데 어떤 녀석이 독차지 하고있다는 것이 문제가 되는 것임
...
# * Blocking
#   CPU 자원을 계속 붙잡고 놓지않는 상황임

# * Non Blocking
#   Blocking과는 조금 다른데 예로 어떤 순간에는 1000개가 들어오지만
#   3시간 동안 아무런 입력이 없는 경우가 있을 수 있음
#   보편적으로 Polling 방식을 이용해서 Blocking을 하면 매 순간
#   "이거 있어?" "저거 했어?" "지금 어디까지 했어요?" "지금 뭐하고 있나요?" 같은거를 묻는거임
#   Non Blocking 방식은 "나 이거 했다, 여유되면 가져가"
#   조금 정신 없다가 나중에 확인하고 "어 가져갈게, 나는 이거 했어, 필요하면 가져가"
#   또 다른 녀석들이 와서 "ㅇㅋ 확인함, 난 이거했고 저거 했어 가져가"
#   위와 같은 형태로 쪼는 형태가 아닌 구성을 가지게 됨
#   실제로 재밌는 것은 Blocking 방식은 단순 작업에서 효율이 높지만
#   복잡한 작업에서는 Blocking 은 오히려 성능을 대폭 하락시키고 서비스가 터지는 사고를 유발하게 됨
#   이유는 Request를 요청하고 응답하기까지 시간이 오래 걸리면
#   다른 서버의 요청에 대한 응답을 처리하지 못하고 Timeout이 뜨면서 모든 것이 터지는 상황이 발생함
#   axios에 Timeout을 2500(2.5초) 굉장이 길게 줬는데 Blocking 하면 2.5초는 우습게 씹어먹고 다 터짐

# 고로 Event Driven 방식을 사용해서 Non-Blocking 방식으로
# "야 나 지금 이거 했어, 필요하면 가져가" 하고 다른 것을 대기하지 않고 바로 다음 작업에 착수하게 됨
# 사실 지금 우리가 사용하고 있는 애자일 보드나, 이슈 시스템, 슬랙 묶어 높은 것도 이 방식이긴 함
# FastAPI나 Spring Reactive는 이러한 것들을 서포트한다 보면되겠음
