...
class PolynomialRegressionRepositoryImpl(PolynomialRegressionRepository):
    def regressionAnalysis(self, X, y):
        ...
        # 다항 피쳐 변환 후에는 [1, x1, x2, x1^2, x1 * x2, x2^2, ...] 과 같은 형태를 가지게 됨

        # Q. 선형 회귀는 y = x 형태고, 다항 회귀는 100개의 축이 있다면 100차원인가용?
        # A. 우선 기저(축)이 있으면 100차원이 맞습니다.
        #   그런데 우리는 degree=2를 줬기 때문에 2차원에 대한 분석이라 보면 되겠습니다.
        #   degree=3 이면 3차 함수, 4면 4차 함수라 보면 되겠습니다.
        #   여기서 이제 조금 더 나가면 x, y, z가 물리는 경우가 발생할 수 있는데
        #   이런 복잡한 영역은 선형 회귀가 불가합니다. (왜냐하면 비선형적이기 때문입니다)
        #   보편적으로 화학 분석, 재료 분석 과 같은 것들을 할 때 많이 사용합니다.
        #   예로 배터리 만들 때 보면 A랑 B랑 1:1로 섞으면 뭐가 된다 합니다.
        #   근데 막상 대용량으로 만들기 시작하면 1:1 비율이 틀어지게 됩니다.
        #   (이런것들이 대표적인 비선형적인 문제인데 <- 논문 쓰기엔 좋지만 이걸로 문제 해결이 안됨)
        #   이건 인간이 노가다로 비율을 맞춰야합니다.
        #   결론: 비선형 문제는 살짝 비현실적 허상 같은 느낌이 있습니다.
        #   -> 학교에서 편미분 방정식을 배우면 재밌는 사례가 있는데
        #       경계 조건이 딱딱 들어맞으면 나이스하게 풀 수 있습니다.
        #   -> 그러나 현실 문제에서는 저런게 없다!
        ...
        # 새로 뽑은 X_new를 다항 피처로 변환함 [1, x1, x2, x1^2, x1 * x2, x2^2, ...]
