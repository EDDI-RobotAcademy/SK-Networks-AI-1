...
# 대표적인 예가 LRASM 미사일임
# 보편적으로 미사일로 정밀타격을 할 때 군사용 위성을 사용하여 유도함
# 우리가 사용하는 네비용 GPS 와는 정밀도 급이 다름 - 손에 점 찍고 맞출 수 있음
# 그런데 요즘 전자전이 발달하다보니 통신 신호를 교란시킬 수 있음(재밍이라고 함)
...
# 이 당시 록히트 마틴에서 LRASM 미사일을 발표하고 곰돌이 푸가 화냈음
# 사실 요런 것들을 보면 인공지능이 가장 많이 활용되는 분야가 군사 분야라는 것도 알 수 있음
# 생각보다 요즘 K 방산의 위력이 또 어마어마함(수출 굿)


@kmeansRouter.get('/kmeans-test', response_model=KmeansClusterResponseForm)
async def kmeans_cluster_analysis():
    # Scikit Learn 에서 제공하는 Kmeans Cluster를 생성하는 라이브러리
    # 300개의 샘플 데이터를 생성함
    # 4개의 중앙값을 구성함
    # 클러스터(중앙값) 기준의 Standard Deviation(표준 편차)는 0.60
    # 재현율 만땅~
    ...
    # 여기서 성능지표는 중심점으로부터 데이터들이 떨어진 거리값을 의미함
    ...
    # 그러나 세상에서 다양한 사람들을 만날수도 있으니 알아둘 필요는 있음
