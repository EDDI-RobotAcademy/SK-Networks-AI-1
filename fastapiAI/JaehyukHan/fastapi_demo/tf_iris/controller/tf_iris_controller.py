...
async def tfTrainModel():
    ...
    # 훈련 모델 정의
    # Sequential을 통해 모델의 각 Layer를 순차적으로 구성함을 의미함
    ...
    # sigma + exponential 연산 때문
    # 일단 expoenential은 오일러 공식을 생각해봐도 cos(x) + i * sin(x) 형태를 띄기 때문에
    # 테일러 급수 전개를 해서 각각의 항들을 계산하고 팩토리얼 계산을 상수화하여 계산한다 하더라도
    # 최소 10차항까지는 계산해줘야 근사치를 얻을 수 있기 때문에 연산 자체는 무거움
    # 이런 부분에서 GPU가 활약하면 성능을 상당히 많이 개선할 수 있음
    # 이 목적으로 논의되었던 것이 반도체 분야에선 NPU(Neural-network Process Unit)
    # 혹은 DPU(Deep-learning Processing Unit) 라고 불러왔음
    # 현재 DPU / NPU 시장이 가장 큰 업체는 Apple에 해당함
    # 그리고 짐 켈러가 요즘 RISC V를 가지고 뭔가 하려고 해서 Nvidia 거품이 꺼질 전망임
    ...
    #           만약 이런 경우가 나타난다면 relu를 사용하지 말고 tanh를 사용하는 것을 권장함
    ...
    # 50번 반복 학습시키니 iris라는 꽃 분야에 상당한 대가로 거듭나게 훈련시키는 것
