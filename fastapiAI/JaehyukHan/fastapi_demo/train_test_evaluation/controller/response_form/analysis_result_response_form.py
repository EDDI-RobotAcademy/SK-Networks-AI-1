...
# 일단 ResponseForm을 만들었음
# 근데 '왜 만드는지 모르겠음' 이라고 생각할 수 있음
# 실질적으로 Domain이 협력하는 상황에 대해서도 고민해 볼 필요가 있음
# 당장 여기서는 그냥 다 랜덤이고 존재하는 데이터를 가지고 하기 때문에 그냥 효용을 못 느낄 수 있음
# 그러나 order만 생각해봐도 벌써 account와 product를 연동하고 있음
# 이런 경우 order 정보만 주는게 아니라 경우에 따라서 account와 product 정보의 일부를 추출해서 줘야 할 수 있음
...
# 혹은 일부는 빼내야하고 버려야한다.
# 재밌게도 마켓팅이란 것이 이것도 해보고 저것도 해보고 사람들 반응도 보고 하다보면
...
# 고로 ResponseForm 은 도대체 무엇이냐 ? 라는 의문이 여전히 남음
...
# 대표적으로 주문(Order)이라는 것이 역시 그러합니다.
