...
# 제일 중요한 특징이라면 고차원의 데이터를 분석할 때 주요 특징들을 유지하면서
# 데이터를 저차원의 공간으로 보낼 수 있는 매우 훌륭한 기법입니다.

# 이게 무슨 소리냐 ?
# 고차원의 데이터란 것은 결국 데이터를 판정할 때 사용하는 파라미터가 여러 개 있다는 뜻입니다.
# 쉽게 얘기해서 ax^n + bx^(n-1) + cx^(n-2) + dx^(n-3) + ex^(n-4) + ... + zzz^(n-1004) + ... +
# 위와 같은 형태로 표현 되는 케이스를 좀 더 단순화 시켜서 유사한 혹은 동일한 모양을 만들어내는 것이죠.
# 위 케이스는 결국 n차이지만 PCA를 통해서 이 n차원을 n - x 차원으로 떨굴 수 있다는 것이 핵심입니다.
# 설명을 위해 2차원으로 했지만 3차원, 4차원 등등 관여하는 종속 변수가 더 많을 수 있습니다.

# 실제로 PCA 분석은 분산(Variance)을 최대화하는 축(Principal Component)을 찾는 것이 핵심입니다.
...
# 학교에서 Linear Albera(선형대수) 학습 할 때 고유값, 고유벡터는 많이들 들어보셨을 것입니다.
...
# 결론적으로 종속 변수가 너무 많아서 '정신 나가겠다' 할 때 사용하면 좋습니다.
...
async def pcaAnalysis(principalComponenteAnalysisService: PrincipalComponentAnalysisServiceImpl =
                      Depends(injectPrincipalComponentAnalysisService)):

    print(f"controller -> pcaAnalysis()")

    originalData, pcaData, explainedVarianceRatio = await principalComponenteAnalysisService.pcaAnalysis()
    return {
