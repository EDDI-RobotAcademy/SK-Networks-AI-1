...
async def tfTrainModel():
    ...
    global CLASSFICATION_NAME
    CLASSFICATION_NAME = iris.target_names
    ...
    # 활성 함수로 'relu'를 사용하는데 relu는 y = x 와 같은 선형적인 특성을 가지고 있으면 사용하는 편
    ...
    #            아무리 학습을 많이 해도 데이터 자체가 피동을 만든다는 뜻임 (수렴하지 않고)
...
def predict(tfIrisRequestForm: TfIrisRequestForm):
    ...
    predictedClass = CLASSFICATION_NAME[np.argmax(prediction)]
