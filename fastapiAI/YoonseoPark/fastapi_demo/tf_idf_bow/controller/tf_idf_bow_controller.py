...
async def findSimilarityDocument(tfIdfBowRequestForm: TfIdfBowRequestForm,
    ...
    print(f"controller -> findSimilarityDocument(): tfIdBowRequestForm: {tfIdfBowRequestForm}")

    foundSimilarityDomcumentList = tfIdfBowService.findSimilarDocumentList(tfIdfBowRequestForm.userSendMessage)
    return JSONResponse(content=foundSimilarityDomcumentList, status_code=status.HTTP_200_OK)