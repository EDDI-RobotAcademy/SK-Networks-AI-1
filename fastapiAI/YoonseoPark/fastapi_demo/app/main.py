...
# 또한 vue에서의 가변 인자와는 조금 다르게 여기서 가변 인자는 아래와 같이 {item_id} 로 중괄호로 감쌈
...
# 이것도 최소한이고 REST API 특성상 service, repository, controller가 동일하게 필요함
...
# 우리의 경우엔 request_form, request, response, response_form 으로 구성할 것임

# DTO라는 용어가 근본이 없다 생각하는 이유는
# 나중에 구성이 커지게 되면 request도 엄청나게 많아지고 response도 엄청난 분량으로 늘어나게 됨
# 그럼 잠깐 머뭇하면서 이게 요청인가 응답인가 고민하면서 집중이 깨짐
# 아마 우리 프로젝트 하면서도 엄청나게 비대해지는 것을 볼 수 있을 것이라 생각됨

# 그럼 갑자기 궁금한 것이 생김
# request와 request_form의 차이는 뭔가 ?
# response와 response_form의 차이는 ?
# 둘 다 일맥상통한데 controller 쪽을 왔다갔다 하는 녀석들은 form이 붙음
# servicea 쪽에서는 request, response로 사용한다 보면 됨
# 결국 Full DDD를 할 때는 필요한 것임

# 요약하자면 form 이 붙으면 controller <-> service
# form이 없으면 service <-> entity
# 그래서 내부 객체에 toXXXRequest, fromXXXReponseForm 같은 형태가 만들어질 수 있음
