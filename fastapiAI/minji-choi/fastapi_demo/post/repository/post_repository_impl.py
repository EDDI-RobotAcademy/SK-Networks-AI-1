...
class PostRepositoryImpl(PostRepository):
    ...
    async def create(self, post: Post) -> int:
        ...
        async with self.dbPool.acquire() as connection:
            async with connection.cursor() as cur:
                await cur.execute("insert into post(title, content) values (%s, %s)", (post.title, post.content))
                await connection.commit()
                await cur.execute("select last_insert_id()")
                postId = await cur.fetchone() # 값을 하나만 불러올 때 사용 fetch one
                return postId[0]

    async def findById(self, postId: int) -> Optional[Post]:
        async with self.dbPool.acquire() as connection:
            async with connection.cursor() as cur:
                await cur.execute("select id, title, content from post where id=%s", (postId,))
                result = await cur.fetchone()  # 값을 하나만 불러올 때 사용 fetch one
                if result:
