...
export default {
    ...
    methods: {
        async fetchPCAData () {
            const response = await axiosInstance.fastapiAxiosInst.get('/pca-train')
        ...
        createOriginalChart () {
            // html ref에 걸려있는 originalChart와 연결하겠다는 의미 c의 ref연산자 생각하기
        ...
        createPCAChart () {
            ...
            // .text("PCA Data: 
            //      PC1=${(this.explainedVarianceRatio[0] * 100).toFixed(2)}%, 
            //      PC2=${(this.explainedVarianceRatio[1] * 100).toFixed(2)}%")
        }
    }
}
</script>
