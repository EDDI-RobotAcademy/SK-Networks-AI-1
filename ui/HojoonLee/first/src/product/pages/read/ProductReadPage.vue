        ...
        <v-card v-if="product">
            ...
            <v-card-text>
                <v-container>
                    <v-row>
                        <v-col cols="12">
                            <v-text-field v-model="product.ImageName" readonly label="상품명"/>
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col cols="12">
                    ...
                    <v-row>
                        <v-col cols="12">
                            <v-img :src="getProductImageUrl(product.ImageName)" aspect-ratio="1" class="grey lighten-2">
...
export default {
    ...
    methods: {
        ...
        async onAddToCart () {
            console.log('장바구니 추가 버튼 눌렀음')
            try {
                const cartData = {
                    productId: this.product.productId,
                    productName: this.product.productName,
                    productPrice: this.product.productPrice,
                    quantity: 1, //임시로 기본수량 1
                }
