...
export default {
    ...
    async created () {
        try {
            ...
            // fetch는 get 요청과 동일한 의미
            // 비동기(fetch)처리에서 await를 사용하면 데이터를 확실하게 받고 넘어가겠다는 의미
    ...
    methods: {
        drawChart () {
            ...
            const width = this.svgWidth - margin.left - margin.right   //this. 는 vue객체 내부에 데이터가 있을 때 사용
            ...
            svg.append("g")
                    ...
                    .append("circle")
                    .attr("cx", d => xScale(d))
                    .attr("cy", (d, i) => yScale(y[i]))   //색상?
                    .attr("r", 3)   // 반지름
                    .attr("fill", "blue")  // 실선 처리
