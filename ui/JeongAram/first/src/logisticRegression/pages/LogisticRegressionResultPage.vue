...
export default {
    ...
    methods: {
        async fetchLogisticRegressionData () {
            try {
                const response = 
                    await axiosInst.fastapiAxiosInst.get('/logistic-regression')  // 웹페이지 F12에서 result: 값을 보여줌
        ...
        createChart () {
            if (!this.X.length || !this.y.length ||
                ...
                // 위의 svg 부분에 그릴거야 폭이랑 높이 전달해줌, 한번 정리
               ...
               // svg 요소 크기 지정
                ...
                // 결정 경계 (decisionBoundary) 를 파악하기 위한 선 그릴 준비
                const line = d3.line()
                        .x(d => x(d[0]))
                        .y(d => x(d[1]))
