...
export default {
    ...
    methods: {
        createChart() {
            ...
            //내부에 배치하는 View Box를 svg요소에 맞게 스케일
            //그리고 여백을 생성함
            ...
            const xScale = d3.scaleLinear()
                .domain(d3.extent(this.points, d=>d[0]))     //domain에는 데이터가 들어간다.
                .range([0, width]);         //range에는 내가 보여주고 싶은 범주가 들어간다.
            ...
            // Draw points
            // 기존의 circle을 포함하여 모든 circle요소를 선택함(초반에는 선택될 것 없음)
            // data를 통해 실제 포인트를 찍어야 하는 정보를 맵핑
            // append('circle)을 통해 각 데이터에 대해 실제 'circle' 요소를 추가함
            // .attr('cx')  .attr('cy')를 통해 x,y 좌표를 설정함
            // .attr('r')으로 반지름 값 설정
            // .attr('fill', (d,i)=> color(this.labels[i])); 를 통해 데이터 포인트의 색상을 클러스터 라벨에 따라 변경
