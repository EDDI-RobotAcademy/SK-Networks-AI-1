            ...
            <v-card-text>
                <v-container>
                    ...
                    <v-row justify="end">
                        ...
                        <v-col cols="auto">
                            <router-link :to="{ name: 'BoardReadPage' }"> <!-- 수정상태에서 다시 돌아가는 것 "Read" 페이지로 -->
...
export default {
    ...
    methods: {
        ...
        async onModify () {
            ...
            const payload = {
                title: this.title, // 해당 title을 써야 수정된 title. writer는 수정 없으므로 생략
                content : this.content,
                boardId : this.boardId,
            }
            await this.requestModifyBoardToDjango(payload)
            await this.$router.push({ name : 'BoardReadPage', params: { boardId: this.boardId} })
        },
    },
    created () {
