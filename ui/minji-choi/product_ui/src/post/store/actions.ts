...
export type PostActions = {
    reqeustPostListToFastapi(context: ActionContext<PostState, any>): Promise<void>
    requestCreatePostToFastapi(context: ActionContext<PostState, unknown>, 
                               payload: {title: string, content: string }): Promise<number>
    requestPostToFastapi(context: ActionContext<PostState, any>, id: number): Promise<void>
}

const actions: PostActions = {
    async reqeustPostListToFastapi(context: ActionContext<PostState, any>): Promise<void> {
        try {
            ...
            console.error('reqeustPostListToFastapi() 중 에러 발생', error)
            throw error
        }
    },
    async requestCreatePostToFastapi(context: ActionContext<PostState, unknown>, 
        payload: {title: string, content: string }): Promise<number> {
            ...
            try {
                ...
                console.error('requestCreatePostToFastapi() 과정에서 에러 발생!: ', error)
                throw error
        }
    },
