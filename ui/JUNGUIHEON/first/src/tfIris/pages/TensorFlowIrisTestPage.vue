<template>
    <v-container>
        <v-form @submit.prevent="submitForm">
            ...
            <v-btn type="sumbit" color="primary">Predict</v-btn>
        </v-form>
    ...
    methods: {
        async submitForm () {
            try {
                ...
                const response = await axios.post('http://192.168.0.55:33333/tf-predict', {
                    sepal_length, 
                        sepal_width, 
                        petal_length, 
                        petal_width 
                ...
                alert('모델이 훈련되지 않았으니 모델부터 훈련시키세요!')
            }
        },
        async trainModel () {
            await axios.get('http://192.168.0.55:33333/tf-train')
