...
import  { REQUEST_POST_LIST_TO_FASTAPI, REQUEST_POST_TO_FASTAPI } from './mutations-types'

export type PostActions = {
    requestPostListToFastapi(context: ActionContext<PostState, any>): Promise<void>
    requestCreatePostToFastapi(context: ActionContext<PostState, unknown>, payload: { title: string, content: string }): Promise<number>
...
const actions: PostActions = {
    ...
    async requestCreatePostToFastapi(context: ActionContext<PostState, unknown>, payload: { title: string, content: string }): Promise<number> {
    ...
    async requestPostToFastapi(context: ActionContext<PostState, any>, id: number): Promise<void> {
        ...
        try {
            ...
            console.log('data:', res.data)
            const data: Post = res.data
            context.commit(REQUEST_POST_TO_FASTAPI, data)
        } catch (error) {
